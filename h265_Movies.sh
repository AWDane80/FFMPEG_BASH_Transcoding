#!/bin/bash
  ...
  echo "Daily DVR Movies Recordings Batch Transcode Scipt"
  echo "Conversion JOB: CronTab:Movie Recordings on $( date '+%F_%H:%M')"
  echo "Converting The Following .TS MPEG2 Files:"
  find . | grep "\.ts$"
  echo "-----------------------------------------------------------"
exec 3>&1 4>&2
trap 'exec 2>&4 1>&3' 0 1 2 3
exec 1>"/mnt/PlexNFS/Plex/Daily Transcoding/Logs/$( date '+%F_%H:%M')_h265_Movies_log.out" 2>&1
  ...
  echo "Working File: /mnt/PlexNFS/Plex/Daily Transcoding/Working Files/MovieScript_$( date +%F).txt"
  echo "Converting The Following .TS MPEG2 Files:"
  find . | grep "\.ts$"
  echo "------------------------------------------------------"
  touch "/mnt/PlexNFS/Plex/Daily Transcoding/Working Files/MovieScript_$( date +%F).txt"
  shopt -s globstar
  for filename in ./**/*.ts
  do
  ...
  echo "${filename##/}" >> "/mnt/PlexNFS/Plex/Daily Transcoding/Working Files/MovieScript_$( date +%F).txt"
  echo "------------------------------------------------------"
done
