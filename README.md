...
Check out the othger scripts for Movies and Sport in the Repo's file list for their specific setup. You will notice changes to the output log files, working files and minor tweaks to the FFMPEG parameters depending on my specific needs based on source material and long term quality retention.
...
__Note: If you are working on a base Debian host, make sure you have installed mailutils for this to function, that may come with additional security concerns for your specific situation, so consider your options here, if you want to opt out of email all togther simply remove (or comment out) the mail.sh call in the main script to stop the script from attempting to email you.
```
...
## Scheduling CRONJOB
Finally we schedule the dailytranscode.sh main script to run at our desired time, in my case 1.00 AM local time:
